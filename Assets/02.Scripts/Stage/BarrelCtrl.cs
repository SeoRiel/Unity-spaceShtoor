...
public class BarrelCtrl : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)    // 충돌 (다른 Collider와 접촉) 발생 시, 호출되는 함수
    {
        if(collision.collider.CompareTag("BULLET"))        // 충돌한 게임 오브젝트가 BULLET일 때
        {
            if(++hitCount == 2)
            {
                ExpBarrel();
   ...
   private void ExpBarrel()                                                                                    // 폭발 효과 처리 함수
    {
        GameObject effect = Instantiate(expEffect, transform.position, Quaternion.identity);  // 폭발 효과 프리팹 동적 생성
        Destroy(effect, 2.0f);
    ...
    private void IndirectDamage(Vector3 pos)                                              // 폭발력 전달 함수
    {
